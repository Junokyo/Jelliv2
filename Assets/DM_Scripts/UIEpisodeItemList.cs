...
public class UIEpisodeItemList : MonoBehaviour
{
	...
	{
		EpisodeNo = episodeNo;
		TextEpisodeNo.text = API.GetText(WordIDs.UIEpisodeItemList_TextStage)+" " + EpisodeNo;
		TextLevelRange.text = $"{(EpisodeNo - 1) * 20 + 1}~{(EpisodeNo - 1) * 20 + 20}" +" "+ API.GetText(WordIDs.UIEpisodeItemList_TextLevelRange);
